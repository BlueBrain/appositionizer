// Note: The contents of this file are largely taken from NEURON, were it is
// called:
//   src/utils/profile/profiler_interface.h
//
// Note that the original contains many more backends, which have been removed here.
...
#if defined(APPOSITIONIZER_CALIPER)
...
struct Instrumentor {
    ...
    inline static void phase_begin(const char* name) {
        run_all_((TProfilerImpl::phase_begin(name), 0)...);
    }
    ...
    inline static void phase_end(const char* name) {
        run_all_((TProfilerImpl::phase_end(name), 0)...);
    }
    ...
    inline static void start_profile() {
        run_all_((TProfilerImpl::start_profile(), 0)...);
    }
    ...
    inline static void stop_profile() {
        run_all_((TProfilerImpl::stop_profile(), 0)...);
    }
    ...
    inline static void init_profile() {
        run_all_((TProfilerImpl::init_profile(), 0)...);
    }
    ...
    inline static void finalize_profile() {
        run_all_((TProfilerImpl::finalize_profile(), 0)...);
    }

  private:
    template <class... Ints>
    inline static void run_all_(Ints...) {
        return;
    }
};
